...
if(isset($_POST["login"])){
    ...
    if (filter_var($username, FILTER_VALIDATE_EMAIL)){
        ...
        $registered=FALSE;
        $result1= mysqli_query($conn, "SELECT * FROM `user` WHERE 1");
        ...
        if($registered!=TRUE){
            echo("<script>alert('No user is registered with this email id');</script>");
        }else{
            if($finaldataarray["password"]==$pass){
                echo("<script>alert('Successfully Logged in');</script>");
                
            }else{
                echo("<script>alert('Password didn't match try with correct password');</script>");
        ...
        if($registered!=TRUE){
            echo("<script>alert('No user is registered with this username');</script>");
        }else{
            if($finaldataarray["password"]==$pass){
                echo("<script>alert('Successfully Logged in');</script>");
                header("location:./logdin.html");
                die();
